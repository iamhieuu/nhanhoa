...
| Hostname | mail.hieucute.id.vn |
| Địa chỉ IP | 192.168.136.131 |
...
echo "search hieucute.id.vn" >> /etc/resolvconf/resolv.conf.d/head
echo "nameserver 192.168.136.131" >> /etc/resolvconf/resolv.conf.d/head
...
zone  hieucute.id.vn {
        type master;
                file "/var/lib/bind/hieucute.id.vn.hosts";
        allow-transfer {
                127.0.0.1;
                localnets;
                };
...
```
$ttl 3600
@      IN      SOA     mail.hieucute.id.vn. root.mail.hieucute.id.vn. (
                        1615364925
                        3600
                        600
                        1209600
                        3600 )
hieucute.id.vn.       IN      NS      mail.hieucute.id.vn.
...
su zimbra
cd /opt/zimbra/jetty/webapps/zimbra/logos
zmprov md hieucute.id.vn zimbraSkinLogoURL /logos/logo1.jpg
...
- Backup: login vào user zimbra để thực hiện chạy lệnh backup. Ví dụ với acc `user1@hieucute.id.vn`

```bash
su - zimbra
zmmailbox -z -m user1@hieucute.id.vn getRestURL "/?fmt=tgz" > /opt/zimbra/backup/backup_user1.tgz
```
...
```bash
su - zimbra
zmmailbox -z -m user1@hieucute.id.vn postRestURL "/?fmt=tgz&resolve=reset" "/opt/zimbra/backup/backup_user1.tgz"
```
